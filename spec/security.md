...
- Lookup de recurso por id é sempre scoped à sessão (`lookupForSession`). `GET /api/workspace/:id` do workspace da sessão → 200; id de outro tenant → 404 vazio. O mesmo vale para cliente, projeto, activity, transição de etapa, item de checklist, validação, aprovação, pendência, lembrete, reunião e template de workflow.
...
- `GET /api/workflow-templates` (collection): outro tenant recebe **lista vazia**. GET por id de outro tenant → **404**.
- Autenticado sem membership válida: **403**. Member tentando editar template de checklist **ou** de workflow do próprio workspace: **403**.
