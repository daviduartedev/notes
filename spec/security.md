...
- Lookup de recurso por id é sempre scoped à sessão (`lookupForSession`). `GET /api/workspace/:id` do workspace da sessão → 200; id de outro tenant → 404 vazio. O mesmo vale para cliente, projeto, activity, transição de etapa, item de checklist, validação, aprovação, pendência e lembrete.
...
- Body não grava `workspaceId` nem `createdAt`. Status de validação só via `POST .../transition`. Status de Approval só via `POST .../decide`. Status de Blocker só via `POST .../decide`. Status de Reminder só via `POST .../decide`. `currentStageId` no PATCH de projeto é ignorado. `status` no PATCH de validação é ignorado. `approverId` no body de Approval é ignorado. `assigneeUserId` no body de Blocker com `assigneeKind=client` é ignorado.
