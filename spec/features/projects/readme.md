...
`/projetos`, `/projetos/:id` (cabeçalho operacional + seção Etapas C2 + **seção Checklists C4** + **seção Validações C5** + **seção Aprovações C6** + **seção Pendências C7** + **seção Lembretes C8**; sem portal do cliente). Lista também em `/clientes/:id`. Board operacional em `/pipeline` (C3); `completed`/`cancelled` não entram no quadro.
