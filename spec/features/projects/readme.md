...
`/projetos`, `/projetos/:id` (cabeçalho operacional + seção Etapas C2 + **seção Checklists C4**; sem abas de validação). Lista também em `/clientes/:id`. Board operacional em `/pipeline` (C3); `completed`/`cancelled` não entram no quadro.
